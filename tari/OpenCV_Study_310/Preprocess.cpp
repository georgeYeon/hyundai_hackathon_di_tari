...
cv::Mat maximizeContrast(cv::Mat &imgGrayscale) {
	...
	cv::morphologyEx(imgGrayscale, imgTopHat, CV_MOP_TOPHAT, structuringElement);   //어두운 배경에서 밝은거 도드라지게 함
	cv::morphologyEx(imgGrayscale, imgBlackHat, CV_MOP_BLACKHAT, structuringElement);   //위의 작업과 반대의 역할

	imgGrayscalePlusTopHat = imgGrayscale + imgTopHat;
	imgGrayscalePlusTopHatMinusBlackHat = imgGrayscalePlusTopHat - imgBlackHat;   //밝은 부분만 남음
